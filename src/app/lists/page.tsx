...
export default function ListsPage() {
  ...
  return (
    <AppShell>
      <PageHeader
        title="People"
        lede="Paste LinkedIn profile URLs. We fill in name, title, company, headline, location, and about from each profile."
      />
      ...
      <div className="mt-6 grid items-start gap-6 lg:grid-cols-2">
        <div className="card p-5">
          ...
          <label className="label mt-4">List name</label>
          <input
          ...
          />
          <label className="label mt-4">LinkedIn URLs</label>
          <textarea
            className="field mt-1.5 h-48 font-mono"
        ...
        <div className="card p-5">
          ...
          {!loaded ? (
            ...
            <p className="empty mt-3">No lists yet. Paste profile URLs, then add them.</p>
          ) : null}
          <ul className="mt-3 space-y-2">
            {visible.map((list) => (
              <li key={list.id} className="flex items-center gap-2">
                <Link
                  href={`/lists/${list.id}`}
                  className="card card-link flex min-w-0 flex-1 items-center justify-between gap-3 rounded-xl px-4 py-3 shadow-none"
                >
