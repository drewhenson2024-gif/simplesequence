...
`POST /mcp` with `X-API-Key`. Tools: `import_leads`, `list_lists`, `get_list`, `update_list`, `remove_lead_from_list`, `delete_list`, `create_sequence`, `update_sequence`, `delete_sequence`, `add_leads_to_sequence`, `get_sequence`, `list_sequences`, `start_sequence`, `pause_sequence`, `resume_sequence`, `connect_status`, `get_inbox`, `reply_inbox`, `stop_lead`, `export_leads`, `get_analytics`, `suggest_learnings`, `apply_learnings`. Import / export / reply / learnings never start a sequence. Sequences stay `draft` until `start_sequence`. Before creating one, ask which people list and which sequence to build. When building steps, ask which actions and any delay between them. No delay means send as soon as the gap and that action’s suggested amount allow. Before starting, ask which sequence and which people, and wait. Connecting LinkedIn is Settings in the browser. `apply_learnings` writes a new draft and never auto-starts. `delete_sequence` is draft-only. `create_sequence` / `update_sequence` take the same step fields as Save on the sequence page.
